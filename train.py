...
parser.add_argument('--learning_rate', type=float, default=0.0006, help='Initial learning rate [default: 0.001]')
...
def train():
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            # Get model and loss 
            pred, end_points = MODEL.get_model(pointclouds_pl, is_training_pl, x_z, x_y, x_x, bn_decay=bn_decay)
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(TRAIN_FILES)):
        ...
        for batch_idx in range(num_batches):
            ...
            rotated_data = provider.rotate_perturbation_point_cloud(current_data[start_idx:end_idx, :, :])
